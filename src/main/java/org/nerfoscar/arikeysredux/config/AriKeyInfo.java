package org.nerfoscar.arikeysredux.config;

import org.nerfoscar.arikeysredux.AriKeysReduxMain;
import org.nerfoscar.arikeysredux.ModifierKey;
import org.nerfoscar.arikeysredux.compat.MythicMobsCompat;
...
public class AriKeyInfo {
	...
	public static @Nullable AriKeyInfo from(ConfigurationSection config) {
		if (config.contains("Id")) {
			NamespacedKey key = NamespacedKey.fromString(config.getString("Id", ""), AriKeysReduxMain.get());
	...
	public boolean runCommand(Player player) {
		...
		if (AriKeysReduxMain.get().papi) cmd = PlaceholderAPI.setPlaceholders(player, cmd);
